...
if (isset($_POST['editar_jogos'])) {
    ...
    if (isset($_FILES['local_imagem']) && $_FILES['local_imagem']['error'] === UPLOAD_ERR_OK) {
        $arquivo_tmp = $_FILES['local_imagem']['tmp_name'];
        $nome_arquivo = $_FILES['local_imagem']['name'];
        ...
        if (move_uploaded_file($arquivo_tmp, $caminho_imagem)) {
            // Se o upload foi bem-sucedido, define o novo caminho da imagem no banco de dados
            $local_imagem = $caminho_imagem;
        } else {
            // Se o upload falhar, exibe uma mensagem de erro
            echo "<script>alert('Erro ao mover a imagem. Por favor, tente novamente.');</script>";
            exit;
        }
    } else {
        $local_imagem = '';
    ...
    if (!empty($nome_jogo)) {
        $updates[] = "nome_jogo = '$nome_jogo'";
    }

    if (!empty($descricao_jogo)) {
        $updates[] = "descricao_jogo = '$descricao_jogo'";
    }

    if (!empty($nota_jogo)) {
        $updates[] = "nota_jogo = '$nota_jogo'";
    }

    if (!empty($statusJogo)) {
        $updates[] = "statusJogo = '$statusJogo'";
    }

    if (!empty($local_imagem)) {
        $updates[] = "local_imagem = '$local_imagem'";
    }

    if (!empty($updates)) {
        $sql .= implode(", ", $updates);
        $sql .= " WHERE id_jogos = '$id_jogos'";
...
<body id="page-top">
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item mx-0 mx-lg-1"><a class="nav-link py-3 px-0 px-lg-3 rounded"
                            href="#registrar">Registrar jogos</a></li>
                    <li class="nav-item mx-0 mx-lg-1"><a class="nav-link py-3 px-0 px-lg-3 rounded"
                            href="#editar">Editar jogos</a></li>
                    <li class="nav-item mx-0 mx-lg-1"><a class="nav-link py-3 px-0 px-lg-3 rounded"
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center text-secondary" scope="col">capa</th>
                    ...
                    <form action="jogos.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-floating mb-3">
                            <input required name="nota_jogo" class="form-control" type="number"
                                placeholder="Dê uma nota ao jogo." data-sb-validations="required" min="0"
                                max="5"></input>
                            <label>Nota do jogo (0 - 5)</label>
                        </div>
                        ...
                        <button style="margin-top: 12px;" name="enviar_jogo" class="btn btn-primary btn-xl"
    ...
    <section class="page-section pt-0" id="editar">
        <div class="container">
            <!-- Contact Section Heading-->
            ...
            <div class="row justify-content-center">
                <div class="p-0 col-lg-8 col-xl-7">
                    <form id="formEditarConteudo" action="jogos.php" method="POST" enctype="multipart/form-data">
                        ...
                        <!-- Upload de Capa para Editar -->
                        <div class="mt-3">
                            <div class="mt-5">
                                <h3 style="color: #6C757D; font-weight: normal;">Upload de Capa</h3>
                                <div class="mb-3">
                                    <label for="formFileEditar" class="form-label">Selecione uma imagem</label>
                                    <input name="local_imagem" class="form-control" type="file" id="formFileEditar"
                                        accept="image/*">
                                </div>
                                <div>
                                    <img class="center" id="previewEditar" src="" alt="Preview da Imagem"
                                        style="max-width: 25%; display: none;">
                                </div>
                            </div>
                        </div>
                        ...
                        <button style="margin-top: 12px;" name="editar_jogos" class="btn btn-primary btn-xl"
                            type="submit">Editar</button>

                        <!-- Novo Botão de Excluir -->
                        <button type="submit" name="excluir_jogos" style="margin-top: 12px;" class="btn btn-primary btn-danger btn-xl"
    ...
    <script>
        ...
        document.getElementById('formFileRegistrar').addEventListener('change', function () {
            previewImage(this, 'previewRegistrar');
        });

        document.getElementById('formFileEditar').addEventListener('change', function () {
            previewImage(this, 'previewEditar');
        });
    </script>
